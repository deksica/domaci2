...
const pitanja = [
    ...
    {
      pitanje: "Koji je kontinenti najveći?",
      odgovori: {
        a: "Azija",
        b: "Afrika",
        c: "Evropa"
    ...
    {
      pitanje: "Koji je službeni jezik u Brazilu?",
      odgovori: {
        a: "Spanski",
        b: "Portugalski",
        c: "Engleski"
    ...
    {
      pitanje: "Teoriju o Heliocentričnom sistemu uspostavio je?",
      odgovori: {
        a: "Galileo Galilej",
        b: "Fernando Magelan",
        c: "Nikola Kopernik"
...
function prikaziRezultat(){
  ...
  pitanja.forEach(function(trenutnoPitanje, pitanjeInd){
   ...
   console.log(odgovoreno);
    ...
    document.getElementsByClassName('pitanje')[pitanjeInd].classList.add('green');
  document.getElementsByClassName('odgovori')[pitanjeInd]
.childNodes.forEach(odg=>{
  if(odg.id===trenutnoPitanje.tacanOdgovor){
    odg.classList.add('greenBackground');
  }
})
   brTacnih++;
    }
    else {
      document.getElementsByClassName('pitanje')[pitanjeInd].classList.add('red');
    document.getElementsByClassName('odgovori')[pitanjeInd]
    .childNodes.forEach(odg=>{
      if(odg.id===trenutnoPitanje.tacanOdgovor){
        odg.classList.add('greenBackground');
      }
      else if(odg.id===odgovoreno){
        if(odgovoreno){
        odg.classList.add('redBackground');
        }
      }
    })
  }
